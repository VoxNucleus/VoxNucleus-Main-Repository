...
function callvoxnucleus(url,callback,name,query)
{
    ...
    if(getElementsByClassName("vn_button").length>0){
        ...
        if(url.indexOf("?")>-1){
            url=url+"&wherefrom="+wherefrom_url;
        }
        else{
            url=url+"?wherefrom="+wherefrom_url;
        }
...
function create_link_from_data(data){
    if(data.isInDatabase){
        ...
        var share_url=vn_share_url+"link="+encodeURI(gathered_data.url);
        if(gathered_data.title)
            share_url=share_url+"&title="+encodeURI(gathered_data.title);
        if(gathered_data.description)
            share_url=share_url+"&description="+encodeURI(gathered_data.description);
        if(gathered_data.tags)
            share_url=share_url+"&tags="+encodeURI(gathered_data.tags);
        return share_url;
    }
}
